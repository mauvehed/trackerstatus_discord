# pylint: disable=C0206  # False positive warnings about .items() usage - we are using .items()
import asyncio
import json
import logging
import os
...
def load_config() -> GuildConfigType:
    ...
    try:
        ...
        logger.info("Creating new config file at %s", CONFIG_FILE)
...
def save_config(config_data: GuildConfigType) -> None:
    """Save the configuration to file atomically.

    Args:
        config_data (GuildConfigType): The configuration to save
...
async def on_ready() -> None:
    ...
    try:
        ...
        synced: Sequence[AppCommand] = await bot.tree.sync()
        print(f"\nSuccessfully synced {len(synced)} command(s):")
        for cmd_sync in synced:
            if hasattr(cmd_sync, "name"):
                print(f"  - /{cmd_sync.name}")
...
async def trackerversion(interaction: discord.Interaction) -> None:
    ...
    embed.add_field(name="Bot Version", value=f"v{VERSION}", inline=True)

    embed.add_field(name="TrackerStatus Library", value=f"v{trackerstatus_version}", inline=True)

    await interaction.response.send_message(embed=embed, ephemeral=True)


@bot.tree.command(
    name="trackeravailable", description="List all available trackers that can be monitored"
)
...
@app_commands.choices(
    tracker=[
        app_commands.Choice(name=f"{code} - {TRACKER_NAMES[code]}", value=code)
        for code in TRACKER_NAMES
    ]
)
async def trackeradd(
    ...
    # Check if user has admin permissions and we're in a guild
    ...
    try:
        ...
        if tracker not in statuses:
            await interaction.followup.send(
                f"Error: Could not get status for {TRACKER_NAMES[tracker]}", ephemeral=True
            )
            return
    except Exception as e:
        await interaction.followup.send(
            f"Error validating tracker {TRACKER_NAMES[tracker]}: {str(e)}", ephemeral=True
        )
        return
    ...
    await interaction.followup.send(
        f"Added tracker {TRACKER_NAMES[tracker]} with alerts in {channel.mention}", ephemeral=True
    )


@bot.tree.command(
    name="trackerremove", description="Remove a tracker from monitoring in a specific channel"
)
@app_commands.describe(
    tracker="The tracker to remove from monitoring", channel="The channel to stop monitoring in"
)
@app_commands.choices(
    tracker=[
        app_commands.Choice(name=f"{code} - {TRACKER_NAMES[code]}", value=code)
...
async def trackerremove(
    ...
    if not interaction.user:
        await interaction.response.send_message(
            "Could not determine the user who ran this command.", ephemeral=True
        )
        return
    ...
    if (
        ...
        await interaction.response.send_message(
            "You must have administrator permissions to use this command.", ephemeral=True
        )
        return
    ...
    if (
        guild_id not in current_config
        or "trackers" not in current_config[guild_id]
        or tracker not in current_config[guild_id]["trackers"]
        or current_config[guild_id]["trackers"][tracker]["channel_id"] != channel.id
    ):
        await interaction.response.send_message(
    ...
    try:
        del current_config[guild_id]["trackers"][tracker]
        save_config(current_config)

        # Log the removal
        logger.info(
            f"Tracker removed: {TRACKER_NAMES[tracker]} from guild {interaction.guild.name} "
            f"(#{channel.name}) by {interaction.user.name}"
        )

        await interaction.response.send_message(
            f"Removed tracker {TRACKER_NAMES[tracker]} from {channel.mention}", ephemeral=True
        )
    except Exception as e:
        logger.error(f"Error removing tracker: {e}")
        await interaction.response.send_message(
            "An error occurred while removing the tracker.", ephemeral=True
        )
        ...
        for guild_id, guild_data in config.items():
            ...
            for tracker_name, tracker_data in guild_data["trackers"].items():
                try:
                    ...
                    if (
                        status_code == TrackerStatus.UNSTABLE
                        and tracker_data["last_status"] == TrackerStatus.UNSTABLE.value
                    ):
                        continue
                    ...
                    if (
                        status_code == TrackerStatus.UNSTABLE
                        or tracker_data["last_status"] == TrackerStatus.UNSTABLE.value
                    ):
                    ...
                    if (
                        tracker_data["last_status"] is None
                        or status_code.value != tracker_data["last_status"]
                    ):
                        ...
                        old_status = (
                            "None"
                            if tracker_data["last_status"] is None
                            else (
                                "Online"
                                if tracker_data["last_status"] == TrackerStatus.ONLINE.value
                                else "Offline"
                            )
                        )
...
async def trackerlatency(interaction: discord.Interaction, tracker: str) -> None:
    ...
    try:
        ...
        for service_name, service_data in services.items():
            ...
            embed.add_field(name=service_name, value=f"{status}\nLatency: {latency}ms", inline=True)
        ...
        await interaction.followup.send(
            f"Error getting latency metrics for {TRACKER_NAMES[tracker]}: {str(e)}", ephemeral=True
        )
...
async def trackeruptime(interaction: discord.Interaction, tracker: str) -> None:
    ...
    try:
        ...
        for service_name, service_data in services.items():
            status = "✅ Online" if service_data.get("online") else "❌ Offline"
            embed.add_field(name=service_name, value=status, inline=True)
...
async def trackerrecord(interaction: discord.Interaction, tracker: str) -> None:
    ...
    try:
        ...
        for service_name, service_data in services.items():
            ...
            status = "✅ Online" if service_data.get("online") else "❌ Offline"
            embed.add_field(
                name=service_name, value=f"{status}\nUptime: {uptime} minutes", inline=True
            )
...
@bot.tree.command(
    name="trackerupdate", description="Force an immediate status check of all configured trackers"
)
    ...
    try:
        ...
        for tracker_name, tracker_data in config[guild_id]["trackers"].items():
            try:
                ...
                # Get the channel
                if not interaction.guild:  # Extra safety check
                    continue
                ...
                status_text = (
                    "Online"
                    if status_code == TrackerStatus.ONLINE
                    else ("Unstable" if status_code == TrackerStatus.UNSTABLE else "Offline")
                )
                ...
                # Update the last known status
                config[guild_id]["trackers"][tracker_name].update(
                    {"last_status": status_code.value, "last_check": datetime.now().isoformat()}
                )
                update_count += 1
        ...
        await interaction.followup.send(f"Error performing force update: {str(e)}", ephemeral=True)
